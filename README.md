...
1. Adjust your environment variables (for example via `garmin.env`) so paths like `DB_PATH`, `ROOT_DIR`, and `STATE_DIR` match your mounts. By default the state database will live at `./state/statestore.db` inside the container.
